...
class LucyItemClient:
    ...
    def __init__(self, config: ConfigParser) -> None:
        ...
        self.item_client = LucyItemProvider(config["LUCYITEMS"])
    ...
    def init_client(self) -> None:
        ...
        self.item_client.load_from_recent()
        self._load_items_by_id()
        self.log.debug("Completed. %d items loaded", len(self.item_client))

    def _load_items_by_id(self) -> None:
        """Internal setup only"""
        for item in self.item_client:
    ...
    def search(self, search_term: str, max_results: int = 50) -> list[LucyItem]:
        ...
        search_items = {item.name: item.id for item in self.item_client}
